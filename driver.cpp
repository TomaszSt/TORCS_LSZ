...
void Driver::initTrack(tTrack* t, void *carHandle, void **carParmHandle, tSituation *s)
{
	...
	switch (s->_raceType) {
		case RM_TYPE_PRACTICE:
			snprintf(buffer, BUFSIZE, "drivers/myrobot/%d/practice/%s", INDEX, trackname);
			break;
		case RM_TYPE_QUALIF:
			snprintf(buffer, BUFSIZE, "drivers/myrobot/%d/qualifying/%s", INDEX, trackname);
			break;
		case RM_TYPE_RACE:
			snprintf(buffer, BUFSIZE, "drivers/myrobot/%d/race/%s", INDEX, trackname);
			break;
		default:
			break;
	}
...
void Driver::update(tSituation *s)
{
	...
	distanceToNextTurn = getDistToSegEnd();
	while (tempSeg.type == nextSeg.type) {
		distanceToNextTurn += nextSeg.length;
		nextSeg = *(nextSeg.next);
	}
	currentSegType	= car->_trkPos.seg->type;
	nextSegType		= nextSeg.type;

	avgNextTurnAngle	= 0.0;
	nextArc			= 0.0;
